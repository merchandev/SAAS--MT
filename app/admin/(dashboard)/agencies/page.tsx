...
export default async function AgenciesPage() {
  const agencies = await b2bQueries.getAllAgencies();
  const activeAgencies = agencies.filter((agency) => agency.isActive).length;
  const bookings = agencies.reduce((sum, agency) => sum + agency._count.bookings, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-xs text-gray-500">{activeAgencies} activas</p>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 text-sm font-medium text-gray-500">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-bold">
            {agencies.length > 0
              ? `${(agencies.reduce((sum, agency) => sum + Number(agency.commissionValue), 0) / agencies.length).toFixed(1)}%`
              : "0%"}
          </p>
          <p className="mt-1 text-xs text-gray-500">Segun agencias registradas</p>
        </div>
      </div>
            ...
            {agencies.map((agency) => (
              <tr key={agency.id} className="hover:bg-gray-50/70">
                ...
                <td className="px-5 py-4 text-right">
                  <AgencyRowActions agencyId={agency.id} isActive={agency.isActive} />
                </td>
              </tr>
            ))}
            {agencies.length === 0 && (
