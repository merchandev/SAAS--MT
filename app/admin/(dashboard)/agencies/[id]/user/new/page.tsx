...
export default async function NewAgencyUserPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h3 className="text-2xl font-bold tracking-tight">Usuario de agencia</h3>
          <p className="text-gray-500">{agency.name}</p>
        </div>
      </div>

      <AgencyUserForm agencyId={agency.id} agencyName={agency.name} agencyEmail={agency.email} />
    </div>
  );
}
