...
export default function NewAdminBookingClient({ vehicles }: { vehicles: Vehicle[] }) {
      ...
      <form onSubmit={handleSubmit}>
        <Card>
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="vehicleId">Vehículo asignado</Label>
                <select id="vehicleId" name="vehicleId" className="w-full border rounded p-2" required>
                  <option value="">Selecciona un vehículo</option>
                  {vehicles.map((v) => (
                    <option key={v.id} value={v.id}>
                      {v.name} (Max {v.passengerCapacity} pax)
                    </option>
                  ))}
                </select>
              </div>
              <div className="space-y-2">
                ...
                <select id="tripType" name="tripType" className="w-full border rounded p-2" required>
